...
public class playerController : MonoBehaviour
{
    ...
    {
        var verticalSpeed = playerSettings.MovementForwardSpeed;
        var horizontalSpeed = playerSettings.MovementStrafeSpeed;
        ...
        var newMovementSpeed =
            new Vector3(horizontalSpeed * inputMovement.x * Time.deltaTime,
                0,
                verticalSpeed * inputMovement.y * Time.deltaTime);

        newMovementSpeed =
            cameraHolder.rotation *
            transform.TransformDirection(newMovementSpeed);
    ...
    private void CalculateStance()
    {
        ...
        cameraHeight =
            Mathf
                .SmoothDamp(cameraHolder.localPosition.y,
                currentStance.CameraHeight,
                ref cameraHeightVelocity,
                playerStanceSmoothing);
        cameraHolder.localPosition =
            new Vector3(cameraHolder.localPosition.x,
                cameraHeight,
                cameraHolder.localPosition.z);

        characterController.height =
            Mathf
                .SmoothDamp(characterController.height,
                currentStance.StanceCollider.height,
                ref stanceColiderHeightVelocity,
                playerStanceSmoothing);
        characterController.center =
            Vector3
                .SmoothDamp(characterController.center,
                currentStance.StanceCollider.center,
                ref stanceColiderCenterVelocity,
                playerStanceSmoothing);
    }
    ...
    {
        var start =
            new Vector3(feetTransform.position.x,
        ...
        return Physics
            .CheckCapsule(start, end, characterController.radius, playerMask);
    }
